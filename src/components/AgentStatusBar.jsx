...
function getStatusUI(status, agentName) {
  switch ((status || '').toLowerCase()) {
    /* --- backend strings --- */
    case 'uploaded':
      return { icon: <HourglassEmptyIcon sx={{ color: 'gray' }} />, label: 'Uploaded' };
    case 'classifying':
      return { icon: <CircularProgress size={18} sx={{ color: '#ff6f00' }} />, label: 'IDCA running…' };
    case 'classified':
      return { icon: <CheckCircleIcon sx={{ color: 'limegreen' }} />, label: 'IDCA done' };
    case 'analyzing':
      return { icon: <CircularProgress size={18} sx={{ color: '#ff6f00' }} />, label: 'NAA running…' };
    case 'assessed':
      // 'assessed' means NAA is done, but for AA it means in progress
      if (agentName === 'AA') {
        return { icon: <CircularProgress size={18} sx={{ color: '#ff6f00' }} />, label: 'AA running…' };
      }
      return { icon: <CheckCircleIcon sx={{ color: 'limegreen' }} />, label: 'NAA done' };
    case 'failed':
    ...
    case 'in_progress':
      return { icon: <CircularProgress size={18} sx={{ color: '#ff6f00' }} />, label: 'Working…' };
    case 'done':
  ...
  useEffect(() => {
    agents.forEach(({ name, status }) => {
      ...
      if (isComplete) {
        ...
        // Reset progress when status goes back to pending
        setStartTimes(prev => {
          const newTimes = { ...prev };
          delete newTimes[name];
          return newTimes;
        ...
        return (
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, width: '100%' }}>
              {icon}
              <Typography variant="body2" sx={{ color: '#fff', flex: 1 }}>{label}</Typography>
              {isActive && (
                <Typography variant="caption" sx={{ color: '#ff6f00', fontWeight: 600, minWidth: '45px', textAlign: 'right' }}>
